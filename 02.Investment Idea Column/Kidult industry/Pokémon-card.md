...
수직으로 올라가는 선은 두 가지를 동시에 의미한다. 누군가에게는 *놓친 수익률*이고, 퀀트에게는 *parabolic blow-off의 전형적 형태*다. 이 칼럼은 후자의 관점에서, "추억"이 자산이 될 수 있는가, 그리고 될 수 있다면 어떤 구조에서 그러한가를 세 개의 수집품 시장 — 미국 스포츠카드, 일본 포켓몬, Magic: The Gathering — 으로 나누어 해부한다.
...
즉 net-of-cost IRR은 헤드라인 CAGR보다 구조적으로 훨씬 낮다. 지수는 *측정*이지 *투자가능 수익률*이 아니다. 이것이 "LLM은 엑셀이지 오라클이 아니다"라는 명제의 자산 버전이다. **Card Ladder 지수는 엑셀(과거를 계산한 결과)이지, 미래 수익률을 말하는 오라클이 아니다.**
