...
**금융 용어를 모르는 분들께 해석**
이 퀀트는 금융 섹터에서 수익을 내는 종목을 찾는 시스템이라기보다, "덜 깨지는" 시스템에 가깝다.

백테스트 결과를 보면 다음 사항을 확인할 수 있다.

사분면 전략의 수익률(CAGR)은 Buy & Hold의 절반도 안 됨 (JPM: 3.2% vs 16.7%)
대신 최대손실(MDD)은 3분의 1로 줄어듦 (JPM: -13% vs -43%)
즉, 돈을 더 버는 전략이 아니라, 돈을 덜 잃는 전략이다.

왜 이런 결과 나왔나?

백테스트에 사용된 FQS/OHS는 실제 재무제표 데이터가 아니라 주가 움직임(RSI, 이동평균선)만 보고 만든 가짜 점수라서, 좋은 기업을 진짜로 찾아내지 못함
시장이 하락할 땐 재빨리 현금으로 빠져서 손실을 방어하지만, 반대로 상승할 땐 너무 늦게 진입하거나 아예 못 들어가서 수익을 놓침
진짜 실전 가치는 이 프롬프트에 실제 ROE, NIM, CET1 같은 재무 데이터를 직접 넣고, LLM이 정성적으로 "이 기업은 정말 좋은데 지금 가격이 싼가?"를 판단하게 하는 데 있다. 그렇게 하면 가격만 보는 백테스트보다 훨씬 나은 종목 선별이 가능해진다.

한마디로: "뭘 사야 돈을 버느냐"가 아니라, "지금 사도 망하지 않을 만한 걸 골라주는" 도구라고 보면 된다.
