...
유출된 프롬프트를 바탕으로 Fable 5를 코딩에 최대한 활용하는 방법:

**아티팩트 모드 트리거**
Fable 5는 20줄 이상의 코드에 대해 파일을 생성한다. "아티팩트로 만들어줘" 또는 "파일로 생성해줘"라고 요청하면 다운로드 가능한 아티팩트로 출력된다. 20줄 미만 코드는 채팅에 인라인으로 표시되므로, 저장이 필요하면 "아티팩트로 만들어줘"라고 명시적으로 요청.

**스킬 시스템 활용**
Fable 5는 파일 생성 전 SKILL.md를 읽는다. 코딩 시 타겟 포맷을 명시적으로 언급:
...
```
이 라이브러리들은 테스트 완료되어 보장된다. localStorage/sessionStorage는 아티팩트에서 사용 불가.
...
**네트워크 제한**
허용된 도메인만 egress proxy 통과 가능: `api.anthropic.com`, `github.com`, `pypi.org`, `npmjs.com`, `registry.npmjs.org`, `crates.io`, Ubuntu/보안 미러. 그 외 도메인이 필요하면 사용자에게 네트워크 설정 업데이트를 요청.
...
Fable 5는 인식하지 못하는 게임, 영화, 책, 앨범, 제품, 스포츠 이벤트 등에 대해 반드시 검색 후 답변한다. 2026년 1월 이후 출시된 라이브러리나 프레임워크를 참조할 때는 명시적으로 "먼저 검색해줘"라고 요청.
...
**AI 디자인 트로프 회피**
Claude Design 프롬프트가 명시적으로 경고하는 패턴들:
- 공격적인 그라데이션 배경
- 이모지 (브랜드에 적합한 경우 제외)
...
**GitHub 연동**
GitHub 연결 시 Claude가 레포 구조를 탐색하고 디자인 목업용 파일을 임포트. 색상 토큰, 글로벌 스타일시트, 특정 컴포넌트를 타겟팅하여 픽셀 수준의 충실도 확보.

### 9.3 생산성을 위한 최적화

**메모리 시스템 (옵트인 필요)**
설정에서 "Memory"를 활성화해야 한다. 활성화되면 Claude가 과거 대화에서 파생된 정보를 추출한다. 이는 Past Chats 검색 도구와는 별개.
...
**스타일 커스터마이징**
Fable 5는 UserStyle 모드를 통한 커스텀 스타일 지원:
